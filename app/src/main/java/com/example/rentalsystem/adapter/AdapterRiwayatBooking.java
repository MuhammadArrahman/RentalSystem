...
public class AdapterRiwayatBooking extends RecyclerView.Adapter<AdapterRiwayatBooking.ViewHolder> {
    ...
    public AdapterRiwayatBooking(List<RiwayatBookingModel> listRiwayat) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_riwayat_booking, parent, false);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        // Logika Tombol Dinamis sesuai Gambar
        if (item.getStatus().equalsIgnoreCase("SELESAI")) {
            holder.btnAction.setText("Sewa Lagi");
            holder.btnAction.setBackgroundTintList(ColorStateList.valueOf(Color.parseColor("#2970FF")));
            holder.btnAction.setTextColor(Color.WHITE);
        } else {
            holder.btnAction.setText("Lihat E-Receipt");
            holder.btnAction.setBackgroundTintList(ColorStateList.valueOf(Color.parseColor("#F5F7FA")));
